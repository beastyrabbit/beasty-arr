...
/**
 * Arr webhook accelerator. The URL carries a dedicated webhook-only token;
 * a leaked callback URL cannot authorize any other API action. Any event that
 * touches a series/movie enqueues a targeted refresh; the engine is fully
...
export function registerWebhookRoutes(app: FastifyInstance, ctx: AppContext): void {
  app.post("/api/webhooks/:source", async (request, reply) => {
    ...
    if (typeof token !== "string" || !ctx.webhookToken.verify(token)) {
